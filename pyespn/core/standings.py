...
def get_standings_core(season, standings_type, league_abbv):
    ...
    url = f'http://sports.core.api.espn.com/v2/sports/{api_info["sport"]}/leagues/{api_info["league"]}/seasons/{season}/types/{STANDINGS_TYPE_MAP[league_abbv][standings_type]}/standings?lang=en&region=us'
    response = requests.get(url)
    content = json.loads(response.content)
    records = content['standings']['records']
    athlete_id = get_athlete_id(records['$ref'])
    all_records = []
